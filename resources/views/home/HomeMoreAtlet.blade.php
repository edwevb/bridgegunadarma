					...
					<div class="px-1 mb-5 moreAtlet">
					  <div class="card-borderless shadow h-100 bg-gradient-purple rounded moreAtletShow">
					    ...
					    <div class="card-body">
					      ...
					      <a href="{{ url('/detailAtlet/'.$atlet->id) }}" class="btn bg-purshadow p-3 rounded-circle shadow"><span class="lead font-weight-bold">Detail <i class="fas fa-external-link-alt"></i></span></a>
					    </div>
					  </div> {{-- end card --}}
					</div>
					@endforeach
				</div> {{-- end atlet row --}}
				<a href="{{ url('/') }}" class="btn bg-gradient-purple rounded-pill px-5 shadow btn-none text-white"><span class="lead font-weight-bold">BACK TO HOME <i class="fas fa-home"></i></span></a>
	    </div>
		</div>
  </article>{{-- end atlet --}}
  @section('script')
